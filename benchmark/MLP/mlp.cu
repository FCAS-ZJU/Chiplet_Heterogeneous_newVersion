...
int main(int argc, char** argv)
{
	...
	while(1){
		...
		int res;
		readMessage <<<1,1>>> (srcX,srcY,0,0,Size_A,2,&res);
		readMessage <<<1,1>>> (srcX,srcY,0,0,Size_B,2,&res);
		...
		readMessage <<<1,1>>> (srcX,srcY,0,0,d_dataA,Col_A*Row_A,&res);
		readMessage <<<1,1>>> (srcX,srcY,0,0,d_dataB,Col_B*Row_B,&res);
		...
		passMessage << <1,1>> > (0,0, srcX,srcY,d_dataC,Row_A * Col_B, &res);
		cudaFree(d_dataA);
		cudaFree(d_dataB);
		cudaFree(d_dataC);
