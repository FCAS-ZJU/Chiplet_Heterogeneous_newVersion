...
int main(int argc, char** argv) {
    ...
    for (int round = 0; round < 6; round++) {
        ...
        receiveMessage(idX, idY, locker_x, locker_y, d_dataA, sizeof(int64_t) * Row * Col);
        receiveMessage(idX, idY, locker_x, locker_y, d_dataB, sizeof(int64_t) * Row * Col);
        ...
        sendMessage(locker_x, locker_y, idX, idY, d_dataC, 100 * sizeof(int64_t));
        cudaFree(d_dataA);
        cudaFree(d_dataB);
        cudaFree(d_dataC);
    }
